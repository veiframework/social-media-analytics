...
public class ExcelExportService extends BaseExportService {
    ...
    protected void insertDataToSheet(Workbook workbook, ExportParams entity,
        ...
        try {
            ...
            setCurrentIndex(1);
            createAddressList(workbook, sheet, index, excelParams, 0);
     ...
     * @param excelParams
     */
    private int createAddressList(Workbook workbook, Sheet sheet, int index, List<ExcelExportEntity> excelParams, int cellIndex) {
        int selectIdx = 0;
        for (int i = 0; i < excelParams.size(); i++) {
            if (excelParams.get(i).isAddressList()) {
                ExcelExportEntity entity = excelParams.get(i);
                String[] addressListValues = getAddressListValues(entity);
                selectIdx = selectIdx + 1;
                this.buildSelector(sheet, workbook, addressListValues, index, cellIndex, selectIdx);
            }
            if (excelParams.get(i).getList() != null) {
                cellIndex = createAddressList(workbook, sheet, index, excelParams.get(i).getList(), cellIndex);
            } else {
                cellIndex++;
            }
        }
    ...
    public void buildSelector(Sheet sheet, Workbook workbook, String[] strings, int index, int cellIndex, int selectIdx) {
        String sheetName = "下拉" + index + cellIndex;
        Sheet selectSheet = workbook.createSheet(sheetName);
        workbook.setSheetHidden(selectIdx, true);
        for (int i = 0, length = strings.length; i < length; i++) {
            selectSheet.createRow(i).createCell(0).setCellValue(strings[i]);
        }
        Name category1Name = workbook.createName();
        category1Name.setNameName(sheetName);
        category1Name.setRefersToFormula(sheetName + "!$A$1:$A$" + (strings.length));
        DataValidationHelper helper = sheet.getDataValidationHelper();
        CellRangeAddressList addressList = new CellRangeAddressList(index,
