...
public class RedisService implements ApplicationContextAware {
    ...
    public Boolean setNx(Object key, Object o) {

        return setNx(key, o, 2L, TimeUnit.HOURS);
    }
    ...
    public <T> Map<String, T> getHash(String listKey) {
        BoundHashOperations<Object, String, T> operations = redisTemplate.boundHashOps(listKey);
     ...
     * @return
     */
    public Boolean setNx(Object key, Object o, Long timeout, TimeUnit unit) {
        BoundValueOperations<Object, Object> operations = redisTemplate.boundValueOps(key);
        Boolean b = operations.setIfAbsent(o, timeout, unit);
        log.debug("加锁 " + b + ",键 >>" + key);
        return b;
    }

    public void debounce(Object key, Object o, Long timeout, TimeUnit unit, Runnable runnable) {
        Boolean lock = setNx(key, o, timeout, unit);
        if (!lock) {
            return;
        }
        runnable.run();
    ...
    public <T> T lock(String lockName, Function<Boolean, T> function) {
        return lock(lockName, function, 0);
    }
    ...
    public void setApplicationContext(@NonNull ApplicationContext applicationContext) throws BeansException {
        try {
            Map<String, RedissonClient> beansOfType = applicationContext.getBeansOfType(RedissonClient.class);
            if (MapUtil.isEmpty(beansOfType)) {
                return;
            }
            log.info("find multiple redisson client {}", beansOfType.keySet());
            RedissonClient bizRedisson = beansOfType.get("bizRedisson");
            if (bizRedisson != null) {
                redissonClient = bizRedisson;
            } else {
                redissonClient = new ArrayList<>(beansOfType.values()).get(0);
            }
        } catch (Exception e) {
            log.error("redisson config failed {}", e.getMessage());
        }
    }
}
