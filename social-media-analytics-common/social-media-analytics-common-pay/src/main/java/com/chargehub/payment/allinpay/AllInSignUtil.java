...
public class AllInSignUtil {
    ...
    public static <T> T json2Obj(String jsonstr, Class<T> cls) {
        ...
        T obj = (T) JSONObject.toJavaObject(jsonObject, cls);
        return obj;
    }
    ...
    public static JSONObject validSign(String result, String appkey,
                                       String signType) throws Exception {
        if (StringUtils.isBlank(result)) {
            throw new IllegalArgumentException("allin payment failed, result is empty!");
        }
        JSONObject jsonObject = JSON.parseObject(result);
        if ("SUCCESS".equals(jsonObject.get("retcode"))) {
            TreeMap<String, String> params = new TreeMap<>();
            jsonObject.forEach((k, v) -> params.put(k, String.valueOf(v)));
            validSign(params, appkey, signType);
            return jsonObject;
        }
        throw new IllegalArgumentException("allin payment failed, " + jsonObject.getString("retmsg"));
    }


    public static String unionSign(Map<String, String> params, String appkey,
                                   String signType) throws Exception {
        params.remove("sign");
        if ("MD5".equals(signType)) {// 如果是md5则需要把md5的key加入到排序
