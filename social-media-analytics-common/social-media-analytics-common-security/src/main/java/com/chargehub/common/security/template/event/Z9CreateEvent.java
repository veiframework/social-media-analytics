...
@Setter
@Getter
public class Z9CreateEvent<T extends Z9CrudEntity> implements ResolvableTypeProvider {
    ...
    public Z9CreateEvent(T data, Z9CrudDto<T> dtoData, Class<T> tClass) {
        this.tClass = tClass;
        this.data = Stream.of(data).collect(Collectors.toList());
        this.dtoDatas = Stream.of(dtoData).collect(Collectors.toList());
    }

    public Z9CreateEvent(List<T> data, List<Z9CrudDto<T>> dtoData, Class<T> tClass) {
        this.tClass = tClass;
        this.data = data;
