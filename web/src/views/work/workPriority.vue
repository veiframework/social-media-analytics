...
const getDict = async () => {
  try {
    ...
    const typeRes = await getDicts('work_priority')
...
const getData = async () => {
  ...
  try {
    ...
    const response = await getPriorityListApi(params)
...
const showDetail = async (id) => {
  try {
    const response = await getPriorityApi(id)
    if (response.code === 200) {
...
const handleDelete = async (id) => {
  try {
    ...
    const response = await delPriorityApi(id)
    if (response.code === 200) {
...
const handleSave = (val) => {
  ...
  if (val.id) {
    editPriorityApi(val.id, formData).then(res => resMsg(res))
  } else {
    addPriorityApi(formData).then(res => resMsg(res))
  }
}
