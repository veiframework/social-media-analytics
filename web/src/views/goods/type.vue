...
const topLevelType = ref([]);



const getTopLevelType = async ()=>{
  let query ={}
  query.pageNum = 1;
  query.pageSize = -1;
  query.parentId = -1
  try {
  let res= await listGoodsType(query)
  const list = res.data.records || res.data.list || [];
   topLevelType.value = list.map(i=>({
        label: i.name,
       value: i.id
     }))
  } catch(err){
...
const handleMenu = (val) => {
  ...
  switch (value) {
    ...
    case 'edit':
      ...
      break;
    case 'delete':
      ElMessageBox.confirm('是否确认删除商品分类"' + row.name + '"？', '警告', {
